/*
  # Fix Lobby Game Processing Logic

  1. Problem
    - When timer expires with <2 players, game should be cancelled and new one created
    - Current logic might not be handling this transition properly
    - Need better error handling for cancelled games

  2. Solution
    - Improve process_scheduled_games function
    - Add automatic new game creation after cancellation
    - Better error handling and logging
    - Add function to clean up cancelled games

  3. Changes
    - Enhanced process_scheduled_games function
    - Auto-cleanup of cancelled games
    - Improved ensure_lobby_game_available function
...
BEGIN
  ...
  IF existing_count = 0 THEN
    ...
    BEGIN
      ...
      SELECT game_code, scheduled_start_time 
      INTO new_game_code, new_start_time
      FROM game_sessions 
      WHERE id = new_game_id;
      
      RAISE NOTICE '✅ Created new lobby game % with code % scheduled for %', 
        new_game_id, new_game_code, new_start_time;
      
      RETURN QUERY SELECT 'created'::TEXT, new_game_id, new_game_code, new_start_time;
  ...
  ELSE
    ...
    SELECT id, game_code, scheduled_start_time 
    INTO new_game_id, new_game_code, new_start_time
    FROM game_sessions 
    WHERE status = 'scheduled' 
    AND is_lobby_game = true
    ...
    RETURN QUERY SELECT 'exists'::TEXT, new_game_id, new_game_code, new_start_time;
  END IF;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  IF cleanup_count > 0 THEN
    RAISE NOTICE '🧹 Cleaned up % old lobby games', cleanup_count;
  END IF;
...
BEGIN
  ...
  WHERE gs.id = COALESCE(NEW.session_id, OLD.session_id)
...
BEGIN
  ...
  FOR game_record IN 
    SELECT id, game_code, scheduled_start_time, status, 
           (SELECT COUNT(*) FROM player_boards WHERE session_id = game_sessions.id) as player_count
    FROM game_sessions 
    WHERE is_lobby_game = true 
