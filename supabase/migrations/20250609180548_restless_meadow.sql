/*
  # Add lobby game system to database schema

  1. New Columns
    - `scheduled_start_time` (timestamptz) - When lobby games should auto-start
    - `max_players` (integer) - Maximum players allowed (default 6)
    - `is_lobby_game` (boolean) - Distinguishes lobby games from private games (default true)

  2. Status Updates
    - Add 'scheduled' status for lobby games waiting to start
    - Update constraint to include: scheduled, waiting, active, cancelled, completed

  3. Functions
    - create_lobby_game() - Creates new 6x6 lobby game with 10min countdown
    - process_scheduled_games() - Starts/cancels games based on timer/players
    - ensure_lobby_game_available() - Maintains one scheduled game at all times
    - leave_lobby_game() - Allows players to leave before game starts
    - generate_treasure_positions_6x6() - Creates random treasure positions

  4. Indexes
    - Efficient queries for lobby game status and timing
    - Fast player counting per session
*/
...
-- Function to generate treasure positions for 6x6 grid
CREATE OR REPLACE FUNCTION generate_treasure_positions_6x6(treasure_count INTEGER)
RETURNS JSONB AS $$
DECLARE
...
BEGIN
  ...
  WHILE i < treasure_count LOOP
    row_pos := floor(random() * 6)::INTEGER;
    col_pos := floor(random() * 6)::INTEGER;
